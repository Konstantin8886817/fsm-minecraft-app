...
export default function createFSMLibrary(machineConfig) {
  if (!machineConfig)
    throw new Error('FSM: You must provide machine configuration')
  ...
  function doAction(action) {
    ...
    if (!targetStateConfig)
      throw new Error(`FSM: target state ${targetState} doesn't exist`)

    onActionEnter && onActionEnter.call(this, action, currentState, targetState)
    currentStateConfig.onActionEnter &&
      currentStateConfig.onActionEnter.call(
        this,
        action,
        currentState,
        targetState,
      )
    actionConfig.onActionEnter &&
      actionConfig.onActionEnter.call(this, action, currentState, targetState)

    const prevState = currentState

    actionConfig.action &&
      actionConfig.action.call(this, action, prevState, targetState)
    currentState = targetState

    onActionExit && onActionExit.call(this, action, prevState, targetState)
    currentStateConfig.onActionExit &&
      currentStateConfig.onActionExit.call(this, action, prevState, targetState)
    actionConfig.onActionExit &&
      actionConfig.onActionExit.call(this, action, prevState, targetState)
  ...
  function getReachableStates() {
    const availableActionsConfig = states[currentState].actions || {}
    return uniqueStr(
      Object.values(availableActionsConfig).map(({targetState}) => targetState),
    )
  }
